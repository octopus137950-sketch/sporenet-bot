...
export function handleVoiceLeaveWithFarmStory(
  ...
  if (!session || !session.farmStoryVoiceChannelId || session.farmStoryVoiceChannelId !== oldState.channelId) return;
  if (!session.lastAction || session.lastAction === "voice_leave") return;
